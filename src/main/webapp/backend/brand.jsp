...
<head>
    ...
    <!--  -->
    <title>VTV | Admin - Danh sách thương hiệu</title>
        ...
        <div class="page-header">
            <div class="row align-items-end">
                ...
                <div class="col-sm-auto">
                    <a class="btn btn-primary" href="create_brand">
            ...
            <div class="card-header">
                <div class="row justify-content-between align-items-center flex-grow-1">
                    <div class="col-sm-6 col-md-4 mb-3 mb-sm-0">
                        <form action="list_brands" method="get">
            ...
            <div class="table-responsive datatable-custom">
                <table id="datatable"
                    ...
                    <c:if test="${empty result}">
                        <c:forEach var="brands" items="${listBrands}">
                            <tr>
                                <th>
                                    <a class="d-flex align-items-center" href="edit_brand?id=${brands.id}">
                                        <div class="avatar">
                                            <c:if test="${not empty brands.imagePath}">
                                                <img class="avatar-img"
                                                     src="../images/brand/${brands.id}/${brands.imagePath}"
                                                     alt="Image Description">
                                            </c:if>
                                            <c:if test="${empty brands.imagePath}">
                                                <img class="avatar-img" src="../images/default.jpg"
                                                     alt="Image Description">
                                            </c:if>

                                        </div>
                                        <div class="ml-3">
                                            <span class="h5 text-hover-primary">${brands.name}</span>
                                        </div>
                                    </a>
                                </th>
                                <td>
                                    <c:if test="${brands.enabled}">
                                        <a class="fas fa-check-circle fa-2x icon-green"
                                           href="enable_brand?enabled=false&id=${brands.id}"
                                           title="Vô hiệu hoá">
                                        </a>
                                    </c:if>
                                    <c:if test="${!brands.enabled}">
                                        <a class="fas fa-check-circle fa-2x icon-gray"
                                           href="enable_brand?enabled=true&id=${brands.id}" title="Kích hoạt">
                                        </a>
                                    </c:if>
                                </td>
                                <td>
                    ...
                    <c:if test="${not empty result}">
                        <c:forEach var="brands" items="${result}">
                            <tr>
                                <th>
                                    <a class="d-flex align-items-center" href="edit_brand?id=${brands.id}">
                                        <div class="avatar">
                    ...
                    <nav aria-label="Page navigation example">
                        <ul class="pagination float-right">
                            <li class="${currentPage > 1 ? 'page-item' : 'page-item disabled'}">
                                <a class="page-link"
                                   href="list_brands?pageNumber=${currentPage - 1}&keyword=${keyword}">Trước</a>
                            </li>
                            <c:forEach var="i" begin="1" end="${totalPages}">
                                <li class="${currentPage != i ? 'page-item' : 'page-item active'}">
                                    <a class="page-link"
                                       href="list_brands?pageNumber=${i}&keyword=${keyword}">${i}</a>
                                </li>
                            </c:forEach>
                            <li class="${currentPage < totalPages ? 'page-item' : 'page-item disabled'}">
                                <a class="page-link"
                                   href="list_brands?pageNumber=${currentPage + 1}&keyword=${keyword}">Sau</a>
                            </li>
                        </ul>
                    </nav>
                </div>
...
<script type="text/javascript">
    ...
    function showDeleteConfirmModal(link) {
        ...
        $("#confirmText").text("Bạn có muốn xoá thương hiệu này không ?");
        $("#confirmModal").modal();
    }
